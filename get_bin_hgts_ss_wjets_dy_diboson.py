...
def wald_exponential(x,p):
    ...
    if x <= a:
        wald_exponential = np.sqrt(l/(2*np.pi*arg**3))*ROOT.TMath.Exp(-(l*(arg-mean)**2)/(2*(mean**2)*arg))
        
    elif x>a:
        wald_exponential = np.sqrt(l/(2*np.pi*arg_a**3))*ROOT.TMath.Exp(-(l*(arg_a-mean)**2)/(2*(mean**2)*arg_a))*ROOT.TMath.Exp(-(l*arg_a**2 + 3*arg_a*mean**2 - l*mean**2)*(x-a)/(2*((mean*arg_a)**2)))
    
    return norm*wald_exponential
...
def wald_poly(x,p):
    ...
    if x <= a:
        wald_poly = np.sqrt(l/(2*np.pi*arg**3))*ROOT.TMath.Exp(-(l*(arg-mean)**2)/(2*(mean**2)*arg))
        
    elif x>a:
        wald_poly = np.sqrt(l/(2*np.pi*arg_a**3))*ROOT.TMath.Exp(-(l*(arg_a-mean)**2)/(2*(mean**2)*arg_a))*(1+(x-a)*((l*arg_a**2 + 3*arg_a*mean**2 - l*mean**2)/(2*n*((mean*arg_a)**2))))**(-n)
    
    return norm*wald_poly
...
def crystalball_gauss(x,p):
    ...
    abs_alpha = abs(alpha)
    if arg <= 0:
        crystal_ball_gauss = np.exp(- 0.5 * arg_narrow * arg_narrow)
    elif (arg <= -alpha) & (arg > 0):
        crystal_ball_gauss = np.exp(- 0.5 * arg * arg)
    elif arg > -alpha:
        nDivAlpha = n/abs_alpha
        AA =  np.exp(-0.5 * abs_alpha * abs_alpha)
        B = nDivAlpha - abs_alpha
        #arg_exp = nDivAlpha/(B-arg)
        arg_exp = nDivAlpha/(B+arg)
        crystal_ball_gauss = AA * (arg_exp**n)

    #'''
    return norm*crystal_ball_gauss
...
def plot_counts_mc(init_time, cats, cat_labels, hist, hist_og, x, config, sysunc, ax, plot_bkg_choice):
    ...
    if len(cats) == 6:
        ...
        #'''        
        '''
        #fit_wald_exponential = ROOT.TF1("fit_wald_exponential",wald_exponential,100,1000,5)
        fit_wald_exponential = ROOT.TF1("fit_wald_exponential",wald_exponential,80,1000,5)
        #fit_wald_exponential.SetParameters(1900000, 207, 136, 100, 205)
        fit_wald_exponential.SetParameters(1900000, 207, 136, 80, 205)

        fit_wald_exponential.SetParNames("norm", "lambda", "mean", "shift", "alpha")
        #fit_wald_exponential.FixParameter(0,24000)
        ...
        #fit_wald_exponential.SetParLimits(0,1000,1700)
        #fit_wald_exponential.SetParLimits(0,net_mass.GetMaximum()-10,net_mass.GetMaximum()+10)
        ...
        fit_wald_exponential.SetParLimits(0,1000000,2000000)
        fit_wald_exponential.SetParLimits(1,0,500)
        fit_wald_exponential.SetParLimits(2,80,150)

        fit_wald_exponential.SetParLimits(3,70,79)
        #fit_wald_exponential.SetParLimits(3,70,90)
        #fit_wald_exponential.SetParLimits(3,60,90)
        fit_wald_exponential.SetParLimits(4,200,210)
        #fit_wald_exponential.SetParLimits(4,200,220)
        #fit_wald_exponential.SetParLimits(4,190,210)

        net_mass.Fit("fit_wald_exponential", "R")

        p = fit_wald_exponential.GetParameters()
        ...
        #fit_wald_exponential.SetParameters(p[0], p[1], p[2], p[3], p[4])

        #fit_wald_exponential.SetParameters(2300000, 234, 167, 65, 205)
        fit_wald_exponential.SetParameters(2300000, 234, 167, 75, 210)
        #fit_wald_exponential.SetParameters(2300000, 234, 167, 85, 205)

        fit_wald_exponential.SetLineColor(3)
        fit_wald_exponential.Draw("same")
        print("Drew fit_wald_exponential")
        '''

        '''
        #fit_wald_poly = ROOT.TF1("fit_wald_poly",wald_poly,100,1000,6)
        fit_wald_poly = ROOT.TF1("fit_wald_poly",wald_poly,80,1000,6)
        fit_wald_poly.SetParNames("norm", "lambda", "mean", "shift", "alpha", "power")
        fit_wald_poly.SetParameters(2150000, 228, 157, 72, 210, 3.65)

        #fit_wald_poly.SetParLimits(0,1000,1700)
        #fit_wald_poly.SetParLimits(0,net_mass.GetMaximum()-10,net_mass.GetMaximum()+10)
        #Since the binsize has increased by a factor of 10 (from 2 to 20), thought it was appropriate to change the limits accordingly as well
        #fit_wald_poly.SetParLimits(0,1000000,3000000)
        #fit_wald_poly.SetParLimits(0,1000000,2000000)
        fit_wald_poly.SetParLimits(0,1000000,1900000)
        #fit_wald_poly.SetParLimits(0,1000000,1800000)
        fit_wald_poly.SetParLimits(1,0,500)
        fit_wald_poly.SetParLimits(2,80,200)

        fit_wald_poly.SetParLimits(3,70,79)
        #fit_wald_poly.SetParLimits(3,70,90)
        #fit_wald_poly.SetParLimits(3,60,90)
        fit_wald_poly.SetParLimits(4,200,210)
        #fit_wald_poly.SetParLimits(4,200,220)
        #fit_wald_poly.SetParLimits(4,190,210)
        #fit_wald_poly.SetParLimits(5,0,10)
        fit_wald_poly.SetParLimits(5,0,20)

        net_mass.Fit("fit_wald_poly", "R")

        p = fit_wald_poly.GetParameters()
        
        print("fitting result for crystalball: {parameter1}, {parameter2}, {parameter3}, {parameter4}, {parameter5}, {parameter6}".format(
        parameter1 = p[0],
        parameter2 = p[1],
        parameter3 = p[2],
        parameter4 = p[3],
        ...
        #fit_wald_poly.SetParameters(p[0], p[1], p[2], p[3], p[4], p[5])

        fit_wald_poly.SetParameters(1900000, 425.81, 137.7, 70, 200, 19.1)
        fit_wald_poly.SetLineColor(3)
        fit_wald_poly.Draw("same")
        print("Drew fit_wald_poly")
        '''

        '''
        fit_crystalball = ROOT.TF1("fit_crystalball",crystalball,100,1000,5)
        #fit_crystalball = ROOT.TF1("fit_crystalball",crystalball,80,1000,5)
        fit_crystalball.SetParNames("norm", "mean", "sigma", "alpha", "power")
        fit_crystalball.SetParameters(14000, 172.5, 25, -1, 0.75)
        ...
        fit_crystalball.FixParameter(1,140)
        #fit_crystalball.SetParLimits(1,170,180)
        #fit_crystalball.SetParLimits(1,165,180)
        #fit_crystalball.SetParLimits(1,160,180)
        #fit_crystalball.SetParLimits(1,130,180)
        #fit_crystalball.SetParLimits(1,140,180)
        #fit_crystalball.SetParLimits(1,140,160)
        #fit_crystalball.FixParameter(2,25)

        #fit_crystalball.SetParLimits(2,18,30)
        #fit_crystalball.SetParLimits(2,18,50)
        fit_crystalball.SetParLimits(2,40,100)
        ...
        fit_crystalball.SetParameters(13492, 140, 100, -1.1355, 10)
        ...
        if sysunc is None:
            ...
            #Never actually used this in this particular script
            #'''
            for i in range(0,len(new_x_edges[:-1])):
                print()
        ...
        net_mass = ROOT.TH1D("net_mass","Same sign bkg fit",len(new_x_edges[:-1]),new_x_edges[:-1])
        ...
        '''
        fit_crystalball = ROOT.TF1("fit_crystalball",crystalball,lower_limit_fit,upper_limit,5)
        fit_crystalball.SetParNames("norm", "mean", "sigma", "alpha", "power")
        ...
        #'''
        fit_crystalball_gauss = ROOT.TF1("fit_crystalball_gauss",crystalball_gauss,lower_limit_fit,upper_limit,6)
        fit_crystalball_gauss.SetParNames("norm", "mean", "sigma", "alpha", "power", "narrow_sigma")
        fit_crystalball_gauss.SetParameters(13492, 170, 100, -1.1355, 10, 25)
        fit_crystalball_gauss.SetParLimits(0,net_mass.GetMaximum()-100,net_mass.GetMaximum()+100)
        #fit_crystalball_gauss.FixParameter(1,170)
        fit_crystalball_gauss.FixParameter(1,160)
        fit_crystalball_gauss.SetParLimits(2,40,100)

        fit_crystalball_gauss.SetParLimits(3,-3.9,-0.06)
        fit_crystalball_gauss.SetParLimits(4,0,10)
        fit_crystalball_gauss.SetParLimits(5,0,50)

        net_mass.Fit("fit_crystalball_gauss", "R")

        p = fit_crystalball_gauss.GetParameters()
        ...
        fit_crystalball_gauss.SetParameters(p[0], p[1], p[2], p[3], p[4], p[5])
        fit_crystalball_gauss.SetLineColor(2)
        #fit_crystalball_gauss_gauss.SetParameters(13492, 140, 100, -1.1355, 10, 40)
        #fit_crystalball_gauss.SetLineColor(3)

        fit_crystalball_gauss.Draw("same")
        print("Drew fit_crystalball_gauss")
        #'''
        ...
        for i in range(0,len(new_x_edges[:-1])):
            ...
            current_function = fit_crystalball_gauss.Eval(x_center)
            ...
            if (current_sigma_2 > 0) & (x_center > lower_limit_fit) & (x_center < upper_limit):
        ...
        for i in range(0,len(new_x_edges[:-1])):
            ...
            net_mass_ratio.SetPoint(i+1,net_mass.GetBinCenter(i+1),bar_content[i]/fit_crystalball_gauss.Eval(net_mass.GetBinCenter(i+1)))
            #net_mass_ratio.SetPointError(i+1,net_mass.GetBinCenter(i+1),(max(stat_unc[0][i],stat_unc[1][i]))/fit_crystalball_gauss.Eval(net_mass.GetBinCenter(i+1)))
            net_mass_ratio.SetPointError(i+1,0.0,(max(stat_unc[0][i],stat_unc[1][i]))/fit_crystalball_gauss.Eval(net_mass.GetBinCenter(i+1)))
            #'''
            #Also setting only statistical error at the moment (have removed the extra sqrt which may have been a typo)
            #Do not think there is an option for setting two separate errors for upper and lower ones
            '''
            net_mass_error_ratio.SetBinContent(i+1,bar_content[i]/fit_crystalball_gauss.Eval(net_mass.GetBinCenter(i+1)))
            #Little dicey about this expression - error/data or error/fit?
            net_mass_error_ratio.SetBinError(i+1,(max(stat_unc[0][i],stat_unc[1][i]))/fit_crystalball_gauss.Eval(net_mass.GetBinCenter(i+1)))
            '''
            #'''
            #print("Bin content just before drawing and fitting (rebinned)",net_mass.GetBinContent(i+1))
        ...
        #c1.Print("/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_ttbar_tw_wjets_0508/net_mass_"+ cats[0] + cats[1] + cats[2] + cats[3] + cats[4] + cats[5] + "_wald_polynomial_new.png")     
        #c1.Print("/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_ttbar_tw_wjets_0508/net_mass_"+ cats[0] + cats[1] + cats[2] + cats[3] + cats[4] + cats[5] + "_wald_exponential_new.png")     
        c1.Print("/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_ttbar_tw_wjets_0508/net_mass_"+ cats[0] + cats[1] + cats[2] + cats[3] + cats[4] + cats[5] + "_crystalball_gauss_wjets_dy_diboson.png")     
        c1.Print("/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_ttbar_tw_wjets_0508/net_mass_"+ cats[0] + cats[1] + cats[2] + cats[3] + cats[4] + cats[5] + "_crystalball_gauss_wjets_dy_diboson.C")     

        pad_upper.SetLogy()

        #c1.Print("/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_ttbar_tw_wjets_0508/net_mass_"+ cats[0] + cats[1] + cats[2] + cats[3] + cats[4] + cats[5] + "_wald_polynomial_new_log_y.png")     
        #c1.Print("/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_ttbar_tw_wjets_0508/net_mass_"+ cats[0] + cats[1] + cats[2] + cats[3] + cats[4] + cats[5] + "_wald_exponential_new_log_y.png")     
        c1.Print("/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_ttbar_tw_wjets_0508/net_mass_"+ cats[0] + cats[1] + cats[2] + cats[3] + cats[4] + cats[5] + "_crystalball_gauss_log_y_wjets_dy_diboson.png")     
        c1.Print("/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_ttbar_tw_wjets_0508/net_mass_"+ cats[0] + cats[1] + cats[2] + cats[3] + cats[4] + cats[5] + "_crystalball_gauss_log_y_wjets_dy_diboson.C")     
