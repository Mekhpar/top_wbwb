...
def power_select(fcn_list, index_chi2):
    if index_chi2 > 50: #Length of array - 1
    ...
    status_string = fcn_list[index_chi2][1][0].split()[3]
...
for i in range(start_width,end_width):
    line_number = 0
    line_ch = 0    
    chi_2_file = open(odir + str(i) + "/20GeV_binsize_0605_signal_crystalball_new_j2_b1_net_mass_ss_width_" + str(i) + ".txt", "r")
    ...
    chi2_og_values.append(chi2_values[0])
    ...
    #This is a provision to also include the 'original' fit parameters for lower widths i.e. >= width_17
    '''
    if i < transition_width:
        index_chi2_final = power_select(sorted_fcn_list, 0)
    elif i >= transition_width:
    ...
    for pars in range(par_nums):
        '''
        if i < transition_width:
            parname = sorted_fcn_list[index_chi2_final][1][4+pars].split()[1]
            #print(parname,sorted_fcn_list[index_chi2_final][1][4+pars])
            par_error = sorted_fcn_list[index_chi2_final][1][4+pars].split()[3]
        elif i >= transition_width:
    ...
    for pars in range(par_nums):
        ...
        if i < transition_width:
            parname = sorted_fcn_list[index_chi2_final][1][4+pars].split()[1]
            par_val = float(sorted_fcn_list[index_chi2_final][1][4+pars].split()[2])
        elif i >= transition_width:
            parname = fcn_list[index_chi2_final][4+pars].split()[1]
...
for pars in range(par_nums):
    ...
    ax.legend(handles=h,labels=l,loc='upper right',ncol=2)
    #plt.setp(visible=True)
    plt.savefig('/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_0430_wbj_widths_new/'+ par_names[pars] +'_vs_width_with_errors.png', format='png', bbox_inches='tight')         
...
for pars in range(par_nums):
    ...
    canvas.Print('/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_0430_wbj_widths_new/'+ par_names[pars] +'_vs_width_with_errors_same_sign_root_original.C')
    canvas.Print('/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_0430_wbj_widths_new/'+ par_names[pars] +'_vs_width_with_errors_same_sign_root_original.png')
    canvas.Close() 
'''
#======================================================================================================================================================================

#=======================================Plotting roughly the min chi_squared (it has failed for width 14 but the new one is not far off)=============================
...
x_leeway = 0.5
#y_leeway = 5
y_leeway = 0.1*(max(chi2_og_values) - min(chi2_og_values))

print(width_values)
print(chi2_og_values) #Technically we could have just taken the min, not even sure why this array is theres

#ax.scatter(width_values, chi2_og_values,marker='o', label="Width_"+str(i))

print("Values of xlims before entering grid_tick_plot")
grid_tick_plot([width_values[0]-x_leeway,width_values[-1]+x_leeway],     #xlims
[min(chi2_og_values)-y_leeway,max(chi2_og_values)+y_leeway],           #ylims
[width_values[0],width_values[-1],0.5],                                  #xtick lims and spacing
[min(chi2_og_values),max(chi2_og_values),50],                          #ytick lims and spacing
['major','r','--'],                                                      #x major grid line attributes (which axis, color and linestyle)
['major','r','--'],                                                      #y major grid line attributes (which axis, color and linestyle)
['minor','k','-'],                                                       #x minor grid line attributes (which axis, color and linestyle)
['minor','k','-'],                                                       #y minor grid line attributes (which axis, color and linestyle)
...
print(len(width_values))
print(len(chi2_og_values))

#ax.scatter(width_values, chi2_og_values,marker='o')
ax.scatter(width_values, chi2_og_values,marker='o')
#ax.errorbar(width_values, chi2_og_values, yerr=fit_par_error_values[pars],fmt='none')
h,l=ax.get_legend_handles_labels()
ax.legend(handles=h,labels=l,loc='upper right',ncol=2)
#plt.setp(visible=True)
plt.savefig('/data/dust/user/paranjpe/af-cms.merged/outputs_wbwb/chunk_0430_wbj_widths_new/Chi_squared_vs_width_same_sign_original.png', format='png', bbox_inches='tight')         
print("Saved " + par_names[pars])
plt.close()
