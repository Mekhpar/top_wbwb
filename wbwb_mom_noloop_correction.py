...
class Processor(pepper.ProcessorBasicPhysics):
    ...
    def process_selection_jet_part(self, selector, is_mc, variation, dsname,
        ...
        print("cumulative_ngenpart",cumulative_ngenpart)
        cumulative_ngenpart_shift = np.roll(cumulative_ngenpart, 1)
        print("cumulative_ngenpart_shift",cumulative_ngenpart_shift)
        ...
        print("genpart_flat len",len(genpart_flat))
        print(genpart_flat)
        min_b_match_flat = ak.flatten(min_b_match)
        print("actual_index length",len(actual_index))
        print("min_b_match length",len(min_b_match_flat))
        #genpart_flat_new = np.put(genpart_flat, actual_index, min_b_match_flat)
        np.put(genpart_flat, actual_index, min_b_match_flat)
        print(genpart_flat)
        genpart_mom_new = ak.unflatten(genpart_flat,ak.num(selector.data["GenPart"]))
        print(genpart_mom_new)
        print(len(genpart_mom_new))
        ...
        for i in range(len(selector.data["Jet"])):
            ...
            #==================================Incredibly risky and maybe also long winded and far fetched!!!=========================================
            '''
            #Take combinations, take difference and see where it is zero to equate
            #dR_equ = ak.cartesian([dR_manual_new, min_b_dr], axis=0)
            dR_equ = ak.cartesian([min_b_dr, dR_manual_new], axis=0)
            print(dR_equ)
            dR_equ_mask = (dR_equ[:,"0"]-dR_equ[:,"1"] == 0)
            #dR_equ_mask = ak.unflatten(dR_equ[:,"0"]-dR_equ[:,"1"] == 0,len(dR_manual_new))
            print(dR_equ_mask)

            broadcast_value = -1
            local_index = ak.local_index(dR_equ_mask)
            broadcast = ak.broadcast_arrays(dR_equ_mask, broadcast_value)[1]
            dr_index = ak.where(dR_equ_mask, local_index, broadcast)
            dr_index_inter = dr_index[dr_index != broadcast_value]   
            dr_index_final = dr_index_inter%len(dR_manual_new)
            print(dr_index_inter)
            print(dr_index_final)
            '''
            #==========================================================================================================================================
