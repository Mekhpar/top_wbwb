...
class Processor(pepper.ProcessorBasicPhysics):
    ...
    def top_and_mom_cats(self,is_mc,data):
        ...
        #Making the assumption that there is only a single top (or none for offshell) in each process
        #top_pdgid = data["GenPart"][abs(data["GenPart"].pdgId) == 6].pdgId[:,0]
        ...
        no_bl_mask = (ak.is_none(data["non_b_mother_final"].pdgId)) | (ak.is_none(data["lep_mother_final"].pdgId))

        cats["no_mask"] = no_bl_mask

        cats["top_b_w_l_w"] = top_mask & b_w_l_w
        cats["top_b_w_l_r"] = top_mask & b_w_l_r
        cats["top_b_r_l_w"] = top_mask & b_r_l_w
        cats["top_b_r_l_r"] = top_mask & b_r_l_r

        cats["antitop_b_w_l_w"] = antitop_mask & b_w_l_w
        cats["antitop_b_w_l_r"] = antitop_mask & b_w_l_r
        cats["antitop_b_r_l_w"] = antitop_mask & b_r_l_w
        cats["antitop_b_r_l_r"] = antitop_mask & b_r_l_r
        
        #======================Here b mother will always be 'wrong' so no use in checking [offshell]=========================
        cats["no_top_l_w"] = ~no_bl_mask & no_top_mask & ~l_mask
        ...
        #Does this also require to check that the W and b are coming from the same mother?
        cats["no_top_l_r_lpos"] = ~no_bl_mask & no_top_mask & l_mask & (sign_l_mom[:,0] == 1) #top
        cats["no_top_l_r_lneg"] = ~no_bl_mask & no_top_mask & l_mask & (sign_l_mom[:,0] == -1) #antitop
        #====================================================================================================================

        cats["both_top_b_l_sign_mismatch"] = both_top_mask & b_l_sign_mismatch


        cats["top_b_w_l_w"] = ak.where(ak.is_none(top_mask & b_w_l_w), false_array, top_mask & b_w_l_w)
        cats["top_b_w_l_r"] = ak.where(ak.is_none(top_mask & b_w_l_r), false_array, top_mask & b_w_l_r)
        cats["top_b_r_l_w"] = ak.where(ak.is_none(top_mask & b_r_l_w), false_array, top_mask & b_r_l_w)
        cats["top_b_r_l_r"] = ak.where(ak.is_none(top_mask & b_r_l_r), false_array, top_mask & b_r_l_r)

        cats["antitop_b_w_l_w"] = ak.where(ak.is_none(antitop_mask & b_w_l_w), false_array, antitop_mask & b_w_l_w)
        cats["antitop_b_w_l_r"] = ak.where(ak.is_none(antitop_mask & b_w_l_r), false_array, antitop_mask & b_w_l_r)
        cats["antitop_b_r_l_w"] = ak.where(ak.is_none(antitop_mask & b_r_l_w), false_array, antitop_mask & b_r_l_w)
        cats["antitop_b_r_l_r"] = ak.where(ak.is_none(antitop_mask & b_r_l_r), false_array, antitop_mask & b_r_l_r)

        cats["both_top_b_l_sign_mismatch"] = ak.where(ak.is_none(both_top_mask & b_l_sign_mismatch), false_array, both_top_mask & b_l_sign_mismatch)
        ...
        for i in range(len(data["Lepton"])):
            ...
            print("top_b_w_l_r",cats["top_b_w_l_r"][i])
            print("top_b_r_l_w",cats["top_b_r_l_w"][i])
            print("top_b_r_l_r",cats["top_b_r_l_r"][i])
            print("Antitop onshell Mask values")
            print("antitop_b_w_l_w",cats["antitop_b_w_l_w"][i])
            print("antitop_b_w_l_r",cats["antitop_b_w_l_r"][i])
            print("antitop_b_r_l_w",cats["antitop_b_r_l_w"][i])
            print("antitop_b_r_l_r",cats["antitop_b_r_l_r"][i])
            print("No top (offshell) mask values")
            print("no_top_l_w",cats["no_top_l_w"][i])
            print("Supposed offshell top",cats["no_top_l_r_lpos"][i])
            print("Supposed offshell antitop",cats["no_top_l_r_lneg"][i])
            print("both_top_b_l_sign_mismatch",cats["both_top_b_l_sign_mismatch"][i])
        '''
