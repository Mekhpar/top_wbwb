...
#class TopWidthModel(PhysicsModel):
class TopWidthModel(PhysicsModelBase):
    def __init__(self):
        PhysicsModelBase.__init__(self)  # not using 'super(x,self).__init__' since I don't understand it
        self.widthRange = ["0.001", "10"]  # default
        self.rMode = "nuisance"
        self.mHRange = ["160", "180"]
        self.floatMass = False    

    #Maybe re-defining setPhysicsOptions later (not at the moment)

    def doParametersOfInterest(self):
        POIs = "width"
        if self.rMode == "nuisance":
            #Added back just for debugging
            #self.modelBuilder.out.var("r").setAttribute("flatParam") #Commented because I am pretty sure I do not want the xsec to be constant
            if self.floatMass == False:
                POIs = "width" #Hoping that this will be taken from the definition in the workspace??

        if self.modelBuilder.out.var("width"):
            self.modelBuilder.out.var("width").setRange(float(self.widthRange[0]), float(self.widthRange[1]))
            self.modelBuilder.out.var("width").setConstant(False)
        else:
            self.modelBuilder.doVar(f"width[{self.widthRange[0]},{self.widthRange[1]}]")
